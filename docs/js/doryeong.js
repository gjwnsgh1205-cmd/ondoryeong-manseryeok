...
const Doryeong = (() => {
  ...
  function media(o = {}) {
    ...
    const poster = asset(`assets/video/doryeong-${kind}.jpg`);
...
<svg class="doryeong dr-mode-${mode}${el !== null ? ' dr-el-' + el : ''}" width="${size}"
  ...
  // ---------- 멘트 ----------
  const GREET = '어서 오세요. 저는 마음의 온도를 살피는 온도령이에요.\n오늘은 무엇이 궁금해서 오셨어요?\n보고 싶은 것을 골라도 좋고, 그냥 마음에 걸리는 일을 이야기해도 좋아요.';

  const LOADING = '어디 보자…, 만세력을 넘겨보는 중이에요';

  // 일간별 풀이 도입 대사
  const INTRO_BY_STEM = [
    '음, 곧게 뻗은 큰 나무의 기운이에요. 꺾이는 것보다 멈춰 서는 걸 더 못 견디지 않나요?',             // 甲
    '바람 따라 휘어도 끝내 살아남는 들풀의 기운이에요. 그 부드러움이 실은 힘이에요.',                   // 乙
    '오, 태양의 기운이에요. 들어서면 방이 환해진다는 말, 종종 듣지 않나요?',                            // 丙
    '촛불의 기운이네요. 화려하진 않아도 오래, 깊게 타는 불이에요. 가까운 이들은 알고 있을 거예요.',      // 丁
    '큰 산의 기운이에요. 다들 기대어 쉬어 가죠. 정작 어디에 기대고 계세요?',                            // 戊
    '기름진 밭의 기운이에요. 무엇이든 심으면 자라나게 하는 손을 가졌네요.',                             // 己
    '단단한 강철의 기운이에요. 맺고 끊음이 칼 같겠네요. 그 칼날을 스스로에게 너무 자주 겨누지는 마세요.', // 庚
    '곱게 세공된 보석의 기운이네요. 그 예민한 감각은 재능이자 짐이에요.',                               // 辛
    '넓고 깊은 바다의 기운이에요. 다 품어주는 듯하다가도, 좀처럼 속은 보이지 않죠.',                    // 壬
    '만물을 적시는 이슬비의 기운이에요. 조용히 스며들어 사람을 얻는 분이네요.',                         // 癸
  ];

  // 섹션 도입 브릿지
  const BRIDGES = {
    pillars: '자, 이것이 태어난 순간의 하늘 — 사주 명식이에요.',
    elements: '이번엔 다섯 기운이 어디로 흐르는지 볼게요.',
    daymaster: '이제 어떤 사람인지, 그 중심을 읽어볼게요.',
    daeun: '삶은 십 년마다 계절이 바뀌어요. 지금 지나는 계절을 짚어볼게요.',
    concerns: '요즘 마음이 머무는 곳이 있나요? 골라 보세요. 함께 들여다볼게요.',
  };

  // 오늘의 한 마디 (십성 그룹별)
  const TODAY = {
    '비겁': '오늘은 내 걸음을 지키는 날이에요. 남과 견주지 말고 어제의 나와만 견주어 보세요.',
    '식상': '오늘은 표현이 잘 풀리는 날이에요. 미뤄둔 말, 미뤄둔 글을 꺼내기 좋아요.',
    '재성': '오늘은 눈이 밝아지는 날이에요. 미뤄둔 정리와 실무를 해치우기 좋아요.',
    '관성': '오늘은 책임이 또렷해지는 날이에요. 할 일 하나를 매듭지으면 마음이 한결 가벼워질 거예요.',
    '인성': '오늘은 안을 채우는 날이에요. 배우고, 읽고, 쉬는 것이 곧 나아가는 거예요.',
  };

  const CLOSING = '오늘 풀이는 여기까지예요.\n사주는 정해진 길이 아니라, 앞으로 걸어갈 길의 지도일 뿐 — 지도를 쥔 손은 늘 자기 자신에게 있습니다.\n마음이 많이 무거운 날엔 혼자 견디지 말고, 믿을 만한 사람이나 전문가에게 도움을 청하세요. 부디 평안히 지내세요.';
